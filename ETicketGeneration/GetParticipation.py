...
base_image_path = r'participation certificate.png'
output_folder = 'participation_certificate'
...
positions = {
    'NAME': (1155, 675),  # (center_x, bottom_y)
}

# Helper: Draw text centered at a bottom-center anchor with dynamic sizing
def draw_text_center(draw, text, bottom_center_position, max_width, max_font_size=50, min_font_size=12):
...
with open('participant_names.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        print(f"🎉 Certificate for {row['Participant Name']} saved to {output_path}")
